...
;; helper function to get the emission from a hit
(define (get-emission ray hit)
  (match hit
    [(Hit _ _ _ norm (Material em _)) (em ray norm)]))
...
(define (first-entry hits)
  (match hits
    [(cons hit hitr) (if (= (Hit-t hit) +inf.0) 'None
                         (if (entry? hit) (Some hit) (first-entry hitr)))]
    ['() 'None]))
